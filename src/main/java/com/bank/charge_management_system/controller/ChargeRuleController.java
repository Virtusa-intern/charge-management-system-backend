...
public class ChargeRuleController {
     ...
     * Accessible by all roles including ADMIN
     */
    @GetMapping
     ...
     * POST /api/rules
     * RULE_CREATOR and ADMIN can create rules
     */
    @PostMapping
    @PreAuthorize("hasAnyRole('ADMIN', 'RULE_CREATOR')")
     ...
     * PUT /api/rules/{id}
     * RULE_CREATOR and ADMIN can update rules
     */
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'RULE_CREATOR')")
     ...
     * Delete charge rule
     * DELETE /api/rules/{id}
     * RULE_CREATOR and ADMIN can delete rules
     */
    @DeleteMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'RULE_CREATOR')")
    public ResponseEntity<ApiResponse<String>> deleteRule(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity
                    .ok(ApiResponse.success("Rule deleted successfully", "Rule with id " + id + " has been deleted"));
     ...
     * RULE_APPROVER and ADMIN can approve rules
     */
    @PostMapping("/{id}/approve")
    @PreAuthorize("hasAnyRole('ADMIN', 'RULE_APPROVER')")
     ...
     * RULE_APPROVER and ADMIN can deactivate rules
     */
     ...
     * RULE_APPROVER and ADMIN can reactivate rules
     */
    ...
    /**
     * Get rules statistics - Returns rule performance data
     * GET /api/rules/statistics
     */
    @GetMapping("/statistics")
    public ResponseEntity<ApiResponse<List<ChargeRuleService.RulePerformance>>> getRuleStatistics() {
        try {
            List<ChargeRuleService.RulePerformance> stats = chargeRuleService.getRuleStatistics();
            return ResponseEntity.ok(ApiResponse.success("Rule performance statistics retrieved successfully", stats));
    ...
    public ResponseEntity<ApiResponse<List<ChargeRuleDto>>> getRulesByCategory(@PathVariable String category) {
        try {
            ...
            return ResponseEntity
                    .ok(ApiResponse.success("Rules retrieved successfully for category: " + category, rules));

        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest()
                    .body(ApiResponse.error(
                            "Invalid category: " + category + ". Valid categories: RETAIL_BANKING, CORP_BANKING, ALL",
                            400));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Failed to retrieve rules: " + e.getMessage(), 500));
    ...
    public static class RuleMetadata {
        ...
        // Getters and setters
        public ChargeRule.Category[] getCategories() {
            return categories;
        }

        public void setCategories(ChargeRule.Category[] categories) {
            this.categories = categories;
        }

        public ChargeRule.ActivityType[] getActivityTypes() {
            return activityTypes;
        }

        public void setActivityTypes(ChargeRule.ActivityType[] activityTypes) {
            this.activityTypes = activityTypes;
        }

        public ChargeRule.FeeType[] getFeeTypes() {
            return feeTypes;
        }

        public void setFeeTypes(ChargeRule.FeeType[] feeTypes) {
            this.feeTypes = feeTypes;
        }

        public ChargeRule.ThresholdPeriod[] getThresholdPeriods() {
            return thresholdPeriods;
        }

        public void setThresholdPeriods(ChargeRule.ThresholdPeriod[] thresholdPeriods) {
            this.thresholdPeriods = thresholdPeriods;
        }

        public ChargeRule.Status[] getStatuses() {
            return statuses;
        }

        public void setStatuses(ChargeRule.Status[] statuses) {
