...
@Schema(description = "Request body for creating a new charge rule in the system. All required fields must be provided with valid values.")
public class ChargeRuleCreateRequest {
    ...
    @Schema(description = "Unique rule code identifier", example = "ATMWD001", required = true, maxLength = 10, pattern = "^[0-9A-Z]+$")
    ...
    @Schema(description = "Human-readable name for the rule", example = "ATM Withdrawal Charge", required = true, maxLength = 100)
    ...
    @Schema(description = "JSON object containing rule conditions like amount ranges, customer types, transaction limits, etc.", example = "{\"customerType\": \"RETAIL\", \"minTransactionAmount\": 1000}", required = true)
    ...
    @Schema(description = "Fee value - either percentage (e.g., 2.50 for 2.5%) or flat amount (e.g., 50.00 for ₹50)", example = "2.5000", required = true, minimum = "0", pattern = "^\\d{1,6}(\\.\\d{1,4})?$")
    ...
    @Schema(description = "Currency code for the charge", example = "INR", defaultValue = "INR", maxLength = 3)
    private String currencyCode = "INR";

    @Schema(description = "Minimum charge amount to apply", example = "10.00", defaultValue = "0.00", minimum = "0")
    @DecimalMin(value = "0.0", inclusive = true, message = "Minimum amount must be non-negative")
    private BigDecimal minAmount = BigDecimal.ZERO;

    @Schema(description = "Maximum charge amount cap", example = "500.00")
    private BigDecimal maxAmount;

    @Schema(description = "Number of free transactions before charges apply", example = "5", defaultValue = "0", minimum = "0")
    @Min(value = 0, message = "Threshold count must be non-negative")
    private Integer thresholdCount = 0;

    @Schema(description = "Time period for threshold counting", example = "MONTHLY", defaultValue = "MONTHLY", allowableValues = {
            "DAILY", "WEEKLY", "MONTHLY", "QUARTERLY", "YEARLY" })
    private ChargeRule.ThresholdPeriod thresholdPeriod = ChargeRule.ThresholdPeriod.MONTHLY;

    @Schema(description = "Rule effective start date-time (ISO 8601 format)", example = "2024-01-01T00:00:00", format = "date-time")
    private LocalDateTime effectiveFrom;

    @Schema(description = "Rule effective end date-time (ISO 8601 format)", example = "2024-12-31T23:59:59", format = "date-time")
